...
/* Define REGDEBUG to get debugging output on stdout. */
//#define REGDEBUG
...
int pcre_info (const pcre * external_re, int *optptr, int *first_char)
{
	...
	if (optptr != NULL)
		*optptr = (re->options & PUBLIC_OPTIONS);

	if (first_char != NULL)
		*first_char = ((re->options & PCRE_FIRSTSET) != 0) ?
			re->first_char :
...
static BOOL could_be_empty (uschar * code)
{
	do
	{
          uschar *cc = code + 3;
	...
	else
	{
		...
		switch (c)
		{
     		/* The handling of escape sequences consisting of a string of 
     		   digits starting with one that is not zero is not straight-
     		   forward. By experiment,the way Perl works seems to be as 
     		   follows:
     
     		   Outside a character class, the digits are read as a decimal 
     		   number. If the number is less than 10, or if there are that 
     		   many previous extracting left brackets, then it is a back 
     		   reference. Otherwise, up to three octal digits are read to 
     		   form an escaped byte. Thus \123 is likely to be octal 123 
     		   (cf \0123, which is octal 012 followed by the literal 3). 
     		   If the octal value is greater than 377, the least 
     		   significant 8 bits are taken. Inside a character class, \ 
     		   followed by a digit is always an octal number. */
			...
			case '0':
				c -= '0';
				while (i++ < 2 &&
					  (pcre_ctypes[ptr[1]] & ctype_digit) != 0 &&
					  ptr[1] != '8' &&
					  ptr[1] != '9')
					c = c * 8 + *(++ptr) - '0';
				break;
...
static BOOL is_counted_repeat (const uschar * p)
{
	...
	while ((pcre_ctypes[*p] & ctype_digit) != 0)
		p++;
...
static BOOL compile_branch (int options,
					   int *brackets,
					   uschar ** codeptr,
					   const uschar ** ptrptr,
					   const char **errorptr)
					...
					if (c == '\\')
					{
						...
						else if (c < 0)
						{
							...
							switch (-c)
							{
								case ESC_d:
									for (c = 0; c < 32; c++)
										class[c] |= pcre_cbits[c + cbit_digit];
									continue;
								...
								case ESC_s:
									for (c = 0; c < 32; c++)
										class[c] |= pcre_cbits[c + cbit_space];
									continue;
...
static BOOL is_anchored (register const uschar * code, BOOL multiline)
{
	do
	{
		int   op = (int) code[3];
		...
		{
			if (!is_anchored (code + 3, multiline))
				return FALSE;
		}
		...
		else if (op != OP_SOD && (multiline || op != OP_CIRC))
			return FALSE;
		code += (code[1] << 8) + code[2];
	}
	while (*code == OP_ALT);
...
static BOOL is_startline (const uschar * code)
{
	do
	{
		...
		{
			if (!is_startline (code + 3))
				return FALSE;
		}
		else if (code[3] != OP_CIRC)
			return FALSE;
		code += (code[1] << 8) + code[2];
	}
	while (*code == OP_ALT);
				...
				int *erroroffset)
{
	...
	*erroroffset = 0;
	...
	while ((c = *(++ptr)) != 0)
	{
		...
		switch (c)
		{
			...
			case '(':
				...
				if (ptr[1] == '?')
				{
					switch (c = ptr[2])
					{
						...
						case '#':
							...
							/* Non-referencing groups and lookaheads just 
							 * move the pointer on, and then behave like a 
							 * non-special bracket, except that they don't 
							 * increment the count of extracting brackets. 
							 */
						...
						case '!':
							...
							/* Ditto for the "once only" bracket, allowed 
							 * only if the extra bit is set. 
							 */
						...
						case '>':
							...
							/* Else loop setting valid options until ) 
							 * is met. Anything else is an error. 
							 */
				...
				/* Extracting brackets must be counted so we 
				 * can process escapes in a Perlish way.
				 */

				else
					bracount++;

				/* Non-special forms of bracket. Save length for 
				 * computing whole length at end if there's a repeat 
				 * that requires duplication of the group.
				 */
				...
				/* Handle ket. Look for subsequent max/min; for certain sets 
				 * of values we have to replicate this bracket up to that many 
				 * times. If brastackptr is 0 this is an unmatched bracket 
				 * which will generate an error, but take care not to try to 
				 * access brastack[-1].
				 */
			...
			case ')':
				length += 3;
				{
					int   minval = 1;
					int   maxval = 1;
					int   duplength = (brastackptr > 0) ?
					               length - brastack[--brastackptr] :
					               0;

					/* Leave ptr at the final char; for 
					 * read_repeat_counts this happens automatically; for 
					 * the others we need an increment.
					 */
					...
					/* If there is a minimum > 1 we have to replicate up 
					 * to minval-1 times; if there is a limited maximum 
					 * we have to replicate up to maxval-1 times and allow 
					 * for a BRAZERO item before each optional copy, as we 
					 * also have to do before the first copy if the 
					 * minimum is zero.
					 */
					...
					if ( maxval > minval )
						length += (maxval - minval) * (duplength + 1);
				}
				continue;

				/* Non-special character. For a run of such characters 
				 * the length required is the number of characters + 2, 
				 * except that the maximum run length is 255.
				 * We won't get a skipped space or a non-data escape or 
				 * the start of a # comment as the first character, so 
			...
			default:
				...
				do
				{
					 ...
					 * metacharacter; stop the string before the latter. 
					 */
				...
				/* This "while" is the end of the "do" above. */
				while (runlength < 255 &&
					  (pcre_ctypes[c = *(++ptr)] & ctype_meta) == 0);
	...
	/* Compute the size of data block needed and get it, either from malloc 
	 * or externally provided function. We specify "code[0]" in the 
	 * offsetof() expression rather than just "code", because it has been 
	 * reported that one broken compiler fails on "code" because it is also 
	 * an independent variable. It should make no difference to the value of 
	 * the offsetof().
	 */
	...
	*code++ = OP_END;
	...
	REGEXTRACE ("Length = %d top_bracket = %d top_backref=%d\n",
		   length, re->top_bracket, re->top_backref);

	if (re->options != 0)
	{
		REGEXTRACE ("%s%s%s%s%s%s%s%s\n",
	...
	if ((re->options & PCRE_FIRSTSET) != 0)
	{
		if (isprint (re->first_char))
			REGEXTRACE ("First char = %c\n", re->first_char);
		else REGEXTRACE ("First char = \\x%02x\n", re->first_char);
	}
	...
	while (code < code_end)
	{
		...
		REGEXTRACE ("%3d ", code - code_base);

		if (*code >= OP_BRA)
		{
			REGEXTRACE ("%3d Bra %d", (code[1] << 8) + code[2], *code - OP_BRA);
			code += 2;
		}
		else
		{
			switch (*code)
			{
				...
				case OP_ASSERT_NOT:
				case OP_ONCE:
					REGEXTRACE ("%3d %s", (code[1] << 8) + code[2], OP_names[*code]);
					code += 2;
					break;
				...
				case OP_TYPEMINQUERY:
					if (*code >= OP_TYPESTAR)
						REGEXTRACE ("    %s", OP_names[code[1]]);
					else if (isprint (c = code[1]))
						REGEXTRACE ("    %c", c);
					else	REGEXTRACE ("    \\x%02x", c);
					REGEXTRACE ("%s", OP_names[*code++]);
					break;
      ...
      match_type (int type, int c, BOOL dotall)
{
...
#ifdef REGDEBUG
	if (isprint (c))
		REGEXTRACE ("matching subject %c against ", c);
	else
		REGEXTRACE ("matching subject \\x%02x against ", c);
	REGEXTRACE ("%s\n", OP_names[type]);
#endif
	...
	switch (type)
	{
		case OP_ANY:
			return dotall || c != '\n';
		case OP_NOT_DIGIT:
			return (pcre_ctypes[c] & ctype_digit) == 0;
		case OP_DIGIT:
			return (pcre_ctypes[c] & ctype_digit) != 0;
		case OP_NOT_WHITESPACE:
			return (pcre_ctypes[c] & ctype_space) == 0;
		case OP_WHITESPACE:
			return (pcre_ctypes[c] & ctype_space) != 0;
		case OP_NOT_WORDCHAR:
			return (pcre_ctypes[c] & ctype_word) == 0;
		case OP_WORDCHAR:
			return (pcre_ctypes[c] & ctype_word) != 0;
	}
	return FALSE;
      ...
      match_ref (int number, register const uschar * eptr, int length, match_data * md)
{
	...
	if (eptr >= md->end_subject)
		REGEXTRACE ("matching subject <null>");
	else
	{
		REGEXTRACE ("matching subject ");
		pchars (eptr, length, TRUE, md);
	}
	REGEXTRACE (" against backref ");
		   ...
		   match_data * md)
{
	for (;;)
	{
		...
		switch (*ecode)
		{
			...
			case OP_CUT:
				if (match (eptr, ecode + 1, offset_top, md))
					return TRUE;
			...
			case OP_BRAZERO:
				{
					...
					if (match (eptr, next, offset_top, md))
						return TRUE;
					do
			...
			case OP_BRAMINZERO:
				{
					...
					do
						next += (next[1] << 8) + next[2];
					while (*next == OP_ALT);
					if (match (eptr, next + 3, offset_top, md))
						return TRUE;
					ecode++;
				}
				break;;
			...
			case OP_DOLL:
				if (md->noteol && eptr >= md->end_subject)
					return FALSE;
				if (md->multiline)
				{
			...
			case OP_REF:
				{
					...
					switch (*ecode)
					{
						...
						default:		/* No repeat follows */
							if (!match_ref (number, eptr, length, md))
								return FALSE;
							eptr += length;
			...
			case OP_CHARS:
				{
...
#ifdef REGDEBUG				  /* Sigh. Some compilers never learn. */
					if (eptr >= md->end_subject)
						REGEXTRACE ("matching subject <null> against pattern ");
					else
					{
						REGEXTRACE ("matching subject ");
						pchars (eptr, length, TRUE, md);
						REGEXTRACE (" against pattern ");
					}
					...
					{
						while (length-- > 0)
							if (pcre_lcc[*ecode++] != pcre_lcc[*eptr++])
								return FALSE;
					}
					else
					{
			...
			case OP_MINQUERY:
			   ...
			   REPEATCHAR:
				...
				REGEXTRACE ("matching %c{%d,%d} against subject %.*s\n", c, min, max,
					    max, eptr);
				...
				if (md->caseless)
				{
					...
					else
					{
						...
						for (i = min; i < max; i++)
						{
							if (eptr >= md->end_subject || c != pcre_lcc[*eptr])
								break;
							eptr++;
						}
						while (eptr >= pp)
			...
			case OP_NOTMINQUERY:
			   ...
			   REPEATNOTCHAR:
				...
				REGEXTRACE ("negative matching %c{%d,%d} against subject %.*s\n", c, min, max,
					    max, eptr);
			   ...
			   REPEATTYPE:
				...
				if (min > 0)
					switch (ctype)
					{
						...
						case OP_NOT_DIGIT:
							for (i = 1; i <= min; i++)
								if ((pcre_ctypes[*eptr++] & ctype_digit) != 0)
									return FALSE;
							break;
						...
						case OP_NOT_WHITESPACE:
							for (i = 1; i <= min; i++)
								if ((pcre_ctypes[*eptr++] & ctype_space) != 0)
									return FALSE;
							break;
						...
						case OP_NOT_WORDCHAR:
							for (i = 1; i <= min; i++)
								if ((pcre_ctypes[*eptr++] & ctype_word) != 0)
									return FALSE;
							break;
...
/* If the expression has got more back references than the offsets supplied can
   ...
   of 2. */
	...
	if (re->top_backref > 0 && re->top_backref >= ocount / 2)
	{
		...
		REGEXTRACE (("Got memory to hold back references\n"));
	}
	else
	...
	if (!anchored)
	{
		if ((re->options & PCRE_FIRSTSET) != 0)
		{
			...
			if (match_block.caseless)
				first_char = pcre_lcc[first_char];
		}
...
/* Loop for unanchored matches; for anchored regexps the loop runs just once. */
	...
	do
	{
		...
		else if (!match (start_match, re->code, 2, &match_block))
			continue;
		...
		if (using_temporary_offsets)
		{
			if (offsetcount >= 4)
			{
				...
				REGEXTRACE (("Copied offsets from temporary memory\n"));
			}
			if (match_block.end_offset_top > offsetcount)
				match_block.offset_overflow = TRUE;

			REGEXTRACE (("Freeing temporary memory\n"));
