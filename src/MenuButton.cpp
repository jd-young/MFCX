/*!
     \file     MenuButton.cpp      A button with popup menu.
     \author   ? (don't know - if you recognise this let me know so I can credit 
     you).
     
 -----------------98/08/24 04:53AM-----------------
     Uses code from CodeGuru.Com...

     \section acknowlgements ACKNOWLEDGMENTS
     - The niffy way of using the menu in immediate mode are (directly) from 
       "Using CMenu::TrackPopupMenu as an Immediate Function" by Randy More
       ("http://www.codeguru.com/menu/track_popup.shtml").
     - General ownerdraw format from "Flat owner drawn buttons" by Girish 
       Bharadwaj ("http://www.codeguru.com/buttonctrl/flat_button.shtml").
     - and the idea for drawing the arrow from "PushButton with bitmap & text" 
       by Michael Santoro 
       ("http://www.codeguru.com/buttonctrl/bitmap_text_button.shtml").
     ...
     and include "MenuButton.h" in whatever file you wish to use it in 
     ("stdafx.h" mayhaps).

     Then in the dialog editor, make a normal button, a suggested size is 12x14. 
     The little arrow gfx will be centered in the button however.  Make a 
     handler for a button click, and copy the sample usage code to there, then 
     of course modify to suit your needs.
...
\param	uiIDMenuResource		The resource ID of the menu to pop up.
\param	iMenuIndex			If the menu has several popup items, then 
							this is used to access it.  If omitted this 
							defaults to zero (the first one).
\return	The menu choice resource ID.
*/
DWORD CMenuButton::DoSingleClick (UINT uiIDMenuResource, int iMenuIndex /*= 0*/)
{
    // -[Feral]-----------------------------------------------------------
    // Vars and Assignment
    CMenu menu;
    ...
    CMenu *pmenuPopup = menu.GetSubMenu(iMenuIndex);
...
/// Draws the menu button, with the arrow.
