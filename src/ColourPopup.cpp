...
//
//         11-Jun-99 KS Strings are now loaded from a string resource.
//
//         31-Oct-99 KS Overrode OnEraseBkgnd(...) and made modifications to 
//                      OnPaint(...) to handle flicker free drawing. 
...
//
//         20-Nov-99 JY Modified the default colours to my own preferences.
//                      Added CPersistColourDlg support to remember custom 
//                      colours.
//
// ColourPopup is a helper class for the Colour picker control
// CJColourPicker. Check out the header file or the accompanying 
...
{    
     ...
     { RGB (0xFF, 0xFB, 0xF0),     _T("Magnolia")      }, 
                            ...
                            LPCTSTR szCustomText  /* = NULL */)
{
	...
	if ( szDefaultText == NULL )
		m_strDefaultText = "Automatic";         // Default to English
	else m_strDefaultText = szDefaultText;

	if( szCustomText == NULL )
		m_strCustomText = "Customise";
	else m_strCustomText = szCustomText;
...
void CColourPopup::EndSelection(int nMessage)
{
    ...
    {
        CPersistColourDlg dlg (m_crInitialColour, 
                               CC_FULLOPEN | CC_ANYCOLOR, 
                               this);

        if ( dlg.DoModal() == IDOK )
             m_crColour = dlg.GetColor();
        else nMessage = CPN_SELENDCANCEL;
    } 
    ...
    {
        DestroyWindow();
        m_pParent = NULL;       // We set this to NULL here to make this 
        m_hWnd = NULL;          // method re-entrant.
    }
}
