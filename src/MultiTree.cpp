/**
	...
	\author	Copyright (c) 1999 Richard Hazlewood
	\author	Corrupted by John Young
	
	This code is provided as-is.  Use at your own peril.

	Multi-selection tree. Based, for the most part, on the selection behaviour 
	of the listview control.
	
	TVN_SELCHANGING/TVN_SELCHANGED notifications are used throughout: 
	
		. itemOld For de-selection
		. itemNew for selection.

	Note: 	TVN_SELCHANGING/TVN_SELCHANGED are still sent by default tree 
			processing for focus changes, i.e. a SetItemState passed 
...
BOOL CMultiTree::SetItemState (HTREEITEM hItem, UINT nState, UINT nStateMask)
{
	...
	if ( nStateMask & TVIS_FOCUSED )
	{
		...
		if ( nState & TVIS_FOCUSED )
		{
			...
			{
				// Because SelectItem would de-select the current 'real' 
				// selection (the one with focus), need to make the tree 
				// ctrl think there is no 'real' selection but still keep 
				// the the old item selected.
				...
				CTreeCtrl::SelectItem (NULL);	//will cause notify, but can 
				                              //be taken as focus change
				CTreeCtrl::SetItemState (hFocus, TVIS_SELECTED, TVIS_SELECTED);
				UpdateWindow();
			}

               // set focus (will consequently select, if not already focused)
...
BOOL CMultiTree::Select (HTREEITEM hItem, UINT code)
{
     code;
	if ( m_bMulti ) 
	{
...
BOOL CMultiTree::_SendNotify (LPNMHDR pNMHDR)
{
	...
	BOOL b = m_bEmulated;
	m_bEmulated = TRUE;
	BOOL bRes = ::SendMessage (::GetParent (m_hWnd), WM_NOTIFY, 
...
void CMultiTree::OnButtonDown (BOOL bLeft, UINT nFlags, CPoint point)
{
	...
	if ( !bBase )
	{
		...
		if ( hItem )
		{
			...
			if ( !bBase && bLeft ) 
			{
				...
				{
					// When the tree has check-boxes, the default handler 
					// makes a quick selection of the clicked item, then 
					// re-selects the previously selected item - to cause a 
					// sel-changed notification.  Fortunately it doesn't 
					// affect the multi-selection, so can pass on.
					bBase = (nHF & TVHT_ONITEMSTATEICON);

#ifdef _MST_MULTI_CHECK
					// Use the above define if you want all selected items 
					// to be checked the same when any one of them is 
					// checked - Interestingly this doesn't happen in the 
					// listview control (LVS_EX_CHECKBOXES).
					if ( bBase ) 
					{
						// The default selection notification would mess
						// the multi-selection up, so generate the 
						// notification manually (anyway, this is smoother 
						// than the selection flicker the default gives).
						NMTREEVIEW nmtv;
#ifdef TVN_CHKCHANGE
						...
						if ( IsSelected (hItem) ) 
						{
							...
							while (h) 
							{
								if ( !GetCheck (h) == bChk ) 
								{
									...
									if ( h != hItem ) 
									{    //clicked item will be done last
				...
				// Check if a single selection was clicked again - if so, 
				// then let the base class handle it (this gets us the 
				// default EditLabel() functionality)...
				bBase = ((nHF & TVHT_ONITEMLABEL) != 0) && 
					    GetSelectedCount() == 1 && 
	...
	if ( bBase ) 
	{
		...
		else CTreeCtrl::OnRButtonDown (nFlags, point);
		return;
	}
...
void CMultiTree::DoPreSelection (HTREEITEM hItem, BOOL bLeft, UINT nFlags)
{
	if ( bLeft ) 
	{
		...
		if ( (nFlags & MK_SHIFT) ) 
		{
			...
			SelectRange (m_hSelect, hItem, !(nFlags & MK_CONTROL));
...
BOOL CMultiTree::OnItemExpanding (NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	     ...
	     (pNMTreeView->action == TVE_COLLAPSERESET) ) 
	{
		...
		// because MFC gives this class first crack.  So if changes are 
		// required a derived class will have to be used..
